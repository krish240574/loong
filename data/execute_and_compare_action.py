...
async def compare_results(
        ...
        final_answer: str,
        domain: str = None,
        answer_precision: float = None) -> bool:
    """
    ...
    if domain == "advanced_math" and execution_result is not None and final_answer is not None:
        try:
            ...
            pass
    elif answer_precision:
        if abs(float(execution_result) - float(final_answer)) <= answer_precision:
            return True
        else:
            return False
...
async def process_single_item(item_tuple: Tuple[int, Dict[str, Any]], verifier: PythonVerifier, domain: str = None) -> Tuple[int, Dict[str, Any]]:
    ...
    answer_precision = item.get("metadata", {}).get("answer_precision", 0)
    ...
    if execution_output["execution_successful"]:
        match_status = await compare_results(execution_output["result"], final_answer, domain, answer_precision)
        return idx, {
